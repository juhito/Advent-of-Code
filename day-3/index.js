/* DAY 3
   ...
   Both the oxygen generator rating and the CO2 rating are values that can be found in your diagnostic report -- 
   ...
   - If you only have ony number left, stop; this is the rating value for which you are searching.
   ...
   - Start with all 12 numbers and consider only the first bit of each number. There are more 1 bits (7) than 0 bits (5),
     so keep only the 7 numbers with a 1 in the first position: 11110, 10110, 10111, 10101, 11100, 10000, 11001.
...
function findOxygenRating(array, bit_pos) {
    let zero_count = 0;
    let one_count = 0;
    
    array.forEach((item, index) => {
	parseInt(item[bit_pos]) === 0 ? zero_count++ : one_count++;
    });

    let bit = zero_count > one_count ? 0 : 1;

    let filtered_array = array.filter((item) => {
	return parseInt(item[bit_pos]) === bit;
    });

    if(filtered_array.length > 1) {
	bit_pos++;
	return findOxygenRating(filtered_array, bit_pos);
    }
    else {
...
function findScrubberRating(array, bit_pos) {
    let zero_count = 0;
    let one_count = 0;
    ...
    let bit = zero_count > one_count ? 1 : 0;
    ...
    if(filtered_array.length > 1) {
	bit_pos++;
	return findScrubberRating(filtered_array, bit_pos);
    }
    else {
	return filtered_array[0];
    }
...
function partOne() {
	...
	item.split("").forEach((element, inner_index) => {
	    
	    if(most_common[inner_index] == undefined) {
		most_common[inner_index] = {
		    0: {
			count: 0
		    },
		    1: {
	    ...
	    let obj = most_common[inner_index][element];
	    
	    obj.count += 1;
	});
    });

    for(const [key, value] of Object.entries(most_common)) {
	gamma_rate += value[0].count > value[1].count ? 0 : 1;
	epsilon_rate += value[0].count > value[1].count ? 1 : 0;
    }

    console.log(`Gamma Binary: ${gamma_rate}, Decimal: ${parseInt(gamma_rate, 2)}`);
    console.log(`Epsilon Binary: ${epsilon_rate}, Decimal: ${parseInt(epsilon_rate, 2)}`);
    console.log(`Multiplied: ${parseInt(gamma_rate, 2) * parseInt(epsilon_rate, 2)}`);
}

function partTwo() {
    const oxr = parseInt(findOxygenRating(data, 0), 2);
    const cor = parseInt(findScrubberRating(data, 0), 2);
    const lfs = oxr * cor;

    console.log(`Oxygen Rating: ${oxr}, CO2 Rating: ${cor}, Life Support Rating: ${lfs} `);
}
